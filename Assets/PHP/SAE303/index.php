...
try {
     ...
     throw new \PDOException($e->getMessage(), (int)$e->getCode());
        ...
        // Fonction de conversion de la date AAAAMM au format MM/AAAA pour l'affichage de la date dans les graphiques
        function convertToDate($aaaamm) {
            $annee = substr($aaaamm, 0, 4);
            $mois = substr($aaaamm, 4, 2);
            return $mois . '/' . $annee;
        }
        //Récupération de la station séléctionné dans le formulaire
        if (isset($_GET['selectstation'])) {
            $selectedStation = $_GET['selectstation'];
        } else {
        // Valeur par défaut si aucune station n'est séléctionné
            $selectedStation ='ALLEMOND';

        }
        // Requête pour récupérer les données des stations (Nom, Longitude, Latitude, Altitude)
        $data = $pdo -> prepare("SELECT DISTINCT `NOM_USUEL`,`LON`,`LAT`,`ALTI` FROM `38` ORDER BY `NOM_USUEL` ASC");
        $data -> execute();
        $stations = $data -> fetchAll();
    ?>
    <!-- Formulaire de séléction de la station -->
    <form  class="text-center bg-light shadow"action="index.php">Séléctionnez une station :
        <select name="selectstation" id="station">
            <option value="" disabled selected>Séléctionnez une station</option> 
            <!-- Boucle pour afficher les stations en options -->
            <?php foreach($stations as $ligne): ?>  
                <option value="<?= $ligne['NOM_USUEL']; ?>"><?= $ligne['NOM_USUEL']; ?></option>
            <?php endforeach; ?>
            <!-- Bouton de validation du formulaire -->
            <input type="submit" value="Valider">
            <p>Station séléctionné: <b><?= $selectedStation ?></b></p>
        </select>
    </form>
    <div class="container">
        <div class="row justify-content-center m-1">
            <div class="col-6">
                <h3 class="text-center">Evolution météorologique du département de l'Isère (38)</h3>
            </div>
            <!-- Carte Leaflet affichant toute les stations du département -->
            <div class="row">
                
                <div class="col-6 shadow content">
                    <h5 class="text-center">Affichage de toute les station du département</h5>
                        <!-- Carte Leaflet -->
                        <div id="map"></div>
                        <script>
                            // Initialisation de la carte centrée sur l'Isère
                            var map = L.map('map').setView([45.38862788955813, 5.5001393582145885], 7);
                        ...
                        <script>
                            ...
                            var precipitation = new Chart(ctx, {
                                ...
                                options: {
                                    ...
                                    //Ajout du plugin zoom pour permettre le zoom sur le graphique
                                    plugins: {
                                        zoom: {
                                            zoom: {
                                                wheel: {
                ...
                <!-- Graphique Chart.js pour les températures mensuelle maximales -->
                <div class="content col-6 shadow">
                    <h5>Moyenne mensuelle des températures maximales quotidiennes (en °C et 1/10)</h5>
                    <!-- Ajout du canva qui affichera le graphique Chart.js -->
                        <canvas id="temperature"></canvas>
                        <?php
                        // Requête pour récupérer les données de température maximale de la station séléctionné en prenant uniquement les données valides (Code qualité à 0 ou 1) 
                        $datas=$pdo -> prepare("SELECT NOM_USUEL, AAAAMM, TX, QTX FROM `38` WHERE (QTX = 0 OR QTX = 1) AND TX != '' AND NOM_USUEL= '" . $selectedStation ."' ORDER BY AAAAMM ASC;");
                        $datas -> execute();
			            $results = $datas -> fetchAll( PDO::FETCH_ASSOC);
                        //Initialisation des tableaux pour stocker les dates et les températures maximales
                        $dates=[];
                        $tempmax=[];
                        $displaydate =[];
                        //Boucle pour remplir les tableaux tempmax et displaydate avec les données récupérées
                        foreach($results as $ligne) {
                            // Utilisation de la fonction convertToDate pour formater les dates
                            $displaydate[] = convertToDate($ligne['AAAAMM']);
                            $tempmax[]=$ligne['TX'];
                        }
                        ?>
                        <script>
                            //Création du graphique line Chart.js
                            ...
                            var temperature = new Chart(ctx, {
                                ...
                                data: {
                                    ...
                                    labels: <?= json_encode($displaydate) ?>,
                                    datasets: [{
                                        label: 'Température maximale (en °C et 1/10)',
                ...
                <!-- Graphique Chart.js pour les hauteurs de neige fraîche -->
                <div class="content col-6 shadow">
                    <h5>Cumul mensuel de la hauteur de neige fraîche tombée en 24h (en cm)</h5>
                    <!-- Ajout du canva qui affichera le graphique Chart.js -->
                        <canvas id="neige"></canvas>
                        <?php 
                        // Requête pour récupérer les données de hauteur de neige fraîche de la station séléctionné en prenant uniquement les données valides (Code qualité à 0 ou 1)
                        $datas=$pdo -> prepare("SELECT NOM_USUEL, AAAAMM, HNEIGEFTOT, QHNEIGEFTOT FROM `38` WHERE (QTX = 0 OR QTX = 1) AND TX != '' AND NOM_USUEL= '" . $selectedStation ."' ORDER BY AAAAMM ASC;");
                        $datas -> execute();
			            $results = $datas -> fetchAll( PDO::FETCH_ASSOC);
                        //Initialisation des tableaux pour stocker les dates et les hauteurs de neige mensuelles
                        $neigemax=[];
                        $displaydate = [];
                        //Boucle pour remplir les tableaux neigemax et displaydate avec les données récupérées
                        foreach($results as $ligne) {
                                $neigemax[]=$ligne['HNEIGEFTOT'];
                                // Utilisation de la fonction convertToDate pour formater les dates
                                $displaydate[] = convertToDate($ligne['AAAAMM']);
                        }
                        ?>
                        <script>
                            //Création du graphique line Chart.js
                            var ctx = document.getElementById('neige').getContext('2d');
                            var neige = new Chart(ctx, {
                                type: 'bar',
                                data: {
                                    //Ajout de toute les dates de la requête SQl avec json_encode
                                    labels: <?= json_encode($displaydate) ?>,
                                    datasets: [{
                                        label: 'Hauteur de neige fraîche (en cm)',
                                        backgroundColor: 'rgb(177, 255, 173)',
                                        borderColor: 'rgb(13, 255, 0)',
                                        //Ajout de toute les mesures de la requête SQl avec json_encode
                                        data: <?= json_encode($neigemax) ?>,
                                    }]
                                },
                                options: {
                                    maintainAspectRatio: true,
